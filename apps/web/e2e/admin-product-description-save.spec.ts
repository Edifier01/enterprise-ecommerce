...
test.describe("Admin product description save smoke", () => {
  ...
  test("product edit saves description with stay intent", async ({ page }) => {
    ...
    const productForm = page.locator("form#admin-product-form");
    const descriptionField = productForm.getByLabel("Описание");
    ...
    await expect(productForm.getByRole("alert")).toHaveCount(0);
    ...
    await page.reload();
    await expect(productForm.getByLabel("Описание")).toHaveValue(originalDescription + marker);

    await productForm.getByLabel("Описание").fill(originalDescription);
    await page.getByRole("button", { name: "Сохранить", exact: true }).click();
    await expect(page.getByRole("status").filter({ hasText: "Товар сохранён" })).toBeVisible({
      timeout: 15_000,
    });
